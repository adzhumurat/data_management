...
class Constant:
    ...
    data_dir = os.getenv(
        'ROOT_DATA_DIR',
        os.path.join(current_script_dir, '..', '..', '..', '..', 'data_store')
    )
    if not os.path.exists(data_dir):
        raise RuntimeError(f"Directory does not exist: {data_dir}")
    else:
        print('File list: ', ','.join(os.listdir(data_dir)))
    ...
    child_dirs = (raw_data_dir, )
    # Архив для распаковки - скачать с Google Drive https://drive.google.com/file/d/1g9AJx3ab4yDtpcew97qxcvvW-3ABz6B7/view?usp=drive_link
